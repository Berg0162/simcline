...
const uint16_t ConnectionMinInterval = 6; 	// The minimum connection interval in 1.25ms units. Min interval (7.5 ms)  
const uint16_t ConnectionMaxInterval = 12;	// The maximum connection interval in 1.25ms units. Max interval (15 ms)  
const uint16_t ConnectionLatency = 0;   	  // No slave latency (immediate response)
const uint16_t ConnectionTimeout = 200; 	  // The timeout time in 10ms units before disconnecting. 200 -> 2000 ms
...
const uint16_t ConnectionMTU = 185;     	  // ATT (Attribute Protocol) Maximum Transmission Unit
