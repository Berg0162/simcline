...
const uint16_t ConnectionMinInterval = 24; 	// The minimum connection interval in 1.25ms units. Min interval (7.5 ms)  was  6
const uint16_t ConnectionMaxInterval = 40;	// The maximum connection interval in 1.25ms units. Max interval (15 ms)   was 12
const uint16_t ConnectionLatency = 0;   	  // No slave latency (immediate response)
const uint16_t ConnectionTimeout = 500; 	  // The timeout time in 10ms units before disconnecting. 200 -> 2000 ms     was 200
const uint16_t ScanInterval = 160;		      // The scan interval to use when attempting to connect in 0.625ms units. 160 -> 100 ms
const uint16_t ScanWindow = 64;		          // The scan window to use when attempting to connect in 0.625ms units.    64 ->  40 ms
const uint16_t ConnectionMTU = 100;     	  // ATT (Attribute Protocol) Maximum Transmission Unit                      was 185
...
bool ClientConnectServer() {
  ...
  if(pClient_Tacx == nullptr) { // First time -> create new pClient_Tacx and service database!
      ...
      delay(200); // Give some time to settle
      // First Time Connect to the TACX Trainer (Server/Peripheral)
      hasConnectPassed = pClient_Tacx->connect(trainerDevice->getAddress());
      //hasConnectPassed = pClient_Tacx->connect(trainerDevice, true); // Delete attribute objects and Create service database
            ...
            hasConnectPassed = pClient_Tacx->connect(trainerDevice, false); // Just refresh the service database
    } 
...
void client_Start_Scanning(void) {
  ...
  //pNimBLEScan->setInterval(100); // In milliSeconds 100   org: 1349
  //pNimBLEScan->setWindow(40);   // In milliSeconds   40   org:  449
