...
void client_Connection_Callbacks::onConnect(NimBLEClient* pClient) {
...
#ifdef DEBUG
    DEBUG_PRINT("Current Connection Parameters -> ");
    ...
    DEBUG_PRINTF("ESP32 Client connected to Server device with Name: [%s] MAC Address: [%s][%d]\n", \
...
bool client_Tacx_FEC_Connect(void) {
    ...
    if(pRemote_Tacx_FEC_Rxd_Chr->canNotify()) {
      pRemote_Tacx_FEC_Rxd_Chr->subscribe(true, Tacx_FEC_Rxd_Notify_Callback, true);
    } else {
...
bool ClientConnectServer() {
  ...
  if(pClient_Tacx == nullptr) { // First time -> create new pClient_Tacx and service database!
      ...
      pClient_Tacx->setConnectionParams(ConnectionMinInterval, ConnectionMaxInterval, ConnectionLatency, ConnectionTimeout, \
  ...
  } else if(pClient_Tacx == NimBLEDevice::getDisconnectedClient()) { 
            // Reconnect to the disconnected TACX Trainer (Server/Peripheral) and allow for a streamlined reconnect
            // bool connect(const NimBLEAdvertisedDevice *device, bool deleteAttributes=true, bool asyncConnect=false, bool exchangeMTU=true)
            hasConnectPassed = pClient_Tacx->connect(trainerDevice, false, false, false); // Refresh only and NO CLIENT INITIATED MTU EXCHANGE
    } 
