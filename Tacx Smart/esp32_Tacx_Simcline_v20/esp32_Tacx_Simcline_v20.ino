 ...
 */
  ...
  */
  ...
  if(!NimBLEDevice::isInitialized()) NimBLEDevice::init(THISDEVICENAME);
...
/*
  if (doClientConnectCall) {
    ...
    bool dummy = ClientConnectServer();
  }
...
void client_Connection_Callbacks::onConnect(NimBLEClient* pClient) {
  ...
  Trainer.conn_handle = pClient_Tacx->getConnHandle();
#ifdef DEBUG
...
bool ClientConnectServer() {
  ...
  if(pClient_Tacx == nullptr) { // First time -> create new pClient_Wahoo and service database!
