 ...
 *  
 *  Requirements: Zwift app or alike, operational Simcline and a FTMS/CPS/CSC supporting indoor trainer
...
//#define DEBUG_FTM_TRAININGSTATUS
//#define DEBUG_FTM_STATUS
//#define DEBUG_FTM_CONTROLPOINT_RESPONSE
//#define DEBUG_FTM_CONTROLPOINT_OPCODE_DATA
...
#define TIME_SPAN 2500 // Time span in millis 1000 = 1 second
...
#endif 
  DEBUG_PRINTLN("     <<--  SIMCLINE supporting: CPS, CSC and FTMS   -->>");
  DEBUG_PRINTLN("------------------------- Version 01.1 -------------------------");

#if defined(ARDUINO_NRF52840_FEATHER) 
  DEBUG_PRINTLN("Simcline equiped with: Feather nRF52840");
#endif
#if defined(ARDUINO_NRF52832_FEATHER) 
  DEBUG_PRINTLN("Simcline equiped with: Feather nRF52832");
#endif 
...
void Client_Enable_Notify_Indicate(void)
{
  ...
  if ( client_CP_Measurement_Chr.enableNotify() ) {
    DEBUG_PRINTLN("Enabled 'Notify' for Client CP Measurement values");
  } else {
    DEBUG_PRINTLN("Couldn't enable 'Notify' for Client CP Measurement Characteristic. Increase DEBUG LEVEL for troubleshooting");
  }
  if ( client_CP_ControlPoint_Chr.enableIndicate() ) {
    DEBUG_PRINTLN("Enabled 'Indicate' for Client CP ControlPoint Responses");
  } else {
    DEBUG_PRINTLN("Couldn't enable 'Indicate' for Client CP ControlPoint Characteristic. Increase DEBUG LEVEL for troubleshooting");
  }
  if ( client_CSC_Measurement_Chr.enableNotify() ) {
    DEBUG_PRINTLN("Enabled 'Notify' for Client CSC Measurement values");
  } else  {
    DEBUG_PRINTLN("Couldn't enable notify for Client CSC Measurement Characteristic. Increase DEBUG LEVEL for troubleshooting");
  }
// -------------------------  Enable CP and CSC  Notify and Indicate --------------------------------
// -------------------------  Enable FTMS Notify and Indicate ---------------------------------------  
  if ( client_FTM_TrainingStatus_Chr.enableNotify() ) {
    DEBUG_PRINTLN("Enabled 'Notify' for Client FTM TrainingStatus values");
  } else {
    DEBUG_PRINTLN(">>> Couldn't enable 'Notify' for Client FTM Training Status Characteristic. Increase DEBUG LEVEL for troubleshooting");
  ...
  if ( client_FTM_IndoorBikeData_Chr.enableNotify() ) {
    DEBUG_PRINTLN("Enabled 'Notify' for Client FTM IndoorBikeData values");
  } else {
    DEBUG_PRINTLN(">>> Couldn't enable 'Notify' for Client FTM Indoor Bike Data Characteristic. Increase DEBUG LEVEL for troubleshooting");
    DEBUG_PRINTLN("FTMS (trainer) is controlled by another Client (Training App)!");
    Bluefruit.disconnect(Trainer.conn_handle);
    return;
  }
  if ( client_FTM_ControlPoint_Chr.enableIndicate() ) {
    DEBUG_PRINTLN("Enabled 'Indicate' for Client FTM ControlPoint Response Messages");
  } else {
    DEBUG_PRINTLN(">>> Couldn't enable 'Indicate' for Client FTM Control Point Characteristic. Increase DEBUG LEVEL for troubleshooting");
    DEBUG_PRINTLN("FTMS (trainer) is controlled by another Client (Training App)!");
    Bluefruit.disconnect(Trainer.conn_handle);
    return;
  }
  if ( client_FTM_Status_Chr.enableNotify() ) {
    DEBUG_PRINTLN("Enabled 'Notify' for Client FTM Status values");
  } else {
    DEBUG_PRINTLN(">>> Couldn't enable 'Notify' for Client FTM Status Characteristic. Increase DEBUG LEVEL for troubleshooting");
    DEBUG_PRINTLN("FTMS (trainer) is controlled by another Client (Training App)!");
    Bluefruit.disconnect(Trainer.conn_handle);
    return;
  }
...
// ---> IBD Buffer Data Length depends on data flagged to be present !!!!
  ...
  // Transfer first the contents of data to buffer (array of chars)
  DEBUG_PRINTF(" -> Client Rec'd Raw FTM IBD Data: [%d] [ ", len);
...
//  if ((flags & 2) != 0) { // (flags & 2) --> true or false Average Speed is always(?) there (tested with Elite Direto XR)
    ...
    DEBUG_PRINTF(" Speed: %d KPH", (sim_Speed/100));
    offset += 2;  // UINT16 
//    }
  if ((flags & 4) != 0) {
...
void client_connect_callback(uint16_t conn_handle)
{
  ...
  DEBUG_PRINTLN();
  DEBUG_PRINTLN("Now checking all mandatory Client Services and Characteristics!");
  ...
  if ( client_CyclingPower_Service.discover(conn_handle) )
  {
    ...
    DEBUG_PRINTF("Found it! CPS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
    DEBUG_PRINTLN("Not Found! Disconnecting since Client Cyling Power Service is mandatory!");
  ...
  if ( !client_CP_Measurement_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Disconnecting since Client CP Measurement Characteristic is mandatory!");
    // MANDATORY so disconnect  
    Bluefruit.disconnect(conn_handle);
    return;
  } else {
  ...
  DEBUG_PRINTLN("Not Found! Client CP Control Point characteristic is NOT mandatory!");  
  }
  ...
  for (int i = 0; i < sizeof(client_CP_Feature_Str); i++) {
    ...
    DEBUG_PRINTLN("NOT Found! Client CP Feature characteristic is NOT Mandatory!");
  }
  ...
  // Read 8-bit client CP sensor location value
    ...
    DEBUG_PRINTLN("NOT Found! Client CP Sensor Location characteristic is NOT mandatory!");
  }
  ...
  if ( client_CyclingSpeedCadence_Service.discover(conn_handle) ) //   UUID16_SVC_CYCLING_SPEED_AND_CADENCE
  {
    ...
    uint16_t max_payload = conn->getMtu()-3;
    uint16_t data_length = conn->getDataLength();
    DEBUG_PRINTF("Found it! CSCS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
    DEBUG_PRINTLN("Not Found! Disconnecting since CSC Service is mandatory!");
    // MANDATORY so disconnect  
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  {
    DEBUG_PRINTLN("Not Found! Disconnecting since Client CSC Measurement CHR is mandatory!");
    // MANDATORY so disconnect  
    Bluefruit.disconnect(conn_handle);
    return;
  } else {
  ...
  if ( client_CSC_Location_Chr.discover() ) //   UUID16_CHR_SENSOR_LOCATION
  {
    ...
    DEBUG_PRINTF("Loc#: %d %s\n", client_CSC_Location_Value, client_Sensor_Location_Str[client_CSC_Location_Value]);
      ...
      DEBUG_PRINTLN("Not Found! Client CSC Location Characteristic is NOT mandatory!");
    }
  ...
  if ( client_CSC_Feature_Chr.discover() ) //   UUID16_CHR_CSC_FEATURE
  {
    ...
    if ( (client_CSC_Feature_Flags & (1 << i)) != 0 )
      ...
      DEBUG_PRINTLN("Not Found! Client CSC Feature Characteristic is NOT mandatory!");
  }
  ...
  if ( client_DIS_Service.discover(conn_handle) ) {
    ...
    if ( client_DIS_ManufacturerName_Chr.discover() ) {
      // read and print out Manufacturer
        ...
        DEBUG_PRINT("Client Manufacturer:  "); DEBUG_PRINTLN(client_DIS_Manufacturer_Str);
      }
    }
    //  2
    if ( client_DIS_ModelNumber_Chr.discover() ) {
      ...
      if ( client_DIS_ModelNumber_Chr.read(client_DIS_ModelNumber_Str, sizeof(client_DIS_ModelNumber_Str)) ) { 
        DEBUG_PRINT("Client Model Number:  "); DEBUG_PRINTLN(client_DIS_ModelNumber_Str);
      }
    }
    //  3
    if ( client_DIS_SerialNumber_Chr.discover() ) {
      ...
      if ( client_DIS_SerialNumber_Chr.read(client_DIS_SerialNumber_Str, sizeof(client_DIS_SerialNumber_Str)) ) {
          DEBUG_PRINT("Client Serial Number: "); DEBUG_PRINTLN(client_DIS_SerialNumber_Str);
      }
    }
  }
  
  DEBUG_PRINT("Discovering Client Fitness Machine (FTM) Service ... ");
  ...
  if ( client_FitnessMachine_Service.discover(conn_handle) )
  {
    ...
    uint16_t max_payload = conn->getMtu()-3;
    uint16_t data_length = conn->getDataLength();
    DEBUG_PRINTF("Found it! FTMS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Service is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  if ( client_FTM_Feature_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Feature Characteristic is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  if ( client_FTM_TrainingStatus_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Client FTM Training Status Characteristic is NOT mandatory!");
  }
  ...
  if ( client_FTM_SupportedResistanceLevelRange_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("NOT Found! Disconnecting since Client FTM Supported Resistance Level Range Characteristic is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
...
//    DEBUG_PRINTLN("Not Found! Client FTM Supported Power Range Characteristic is NOT mandatory!"); 
    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Supported Power Range Characteristic is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  if ( client_FTM_IndoorBikeData_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Client FTM Indoor Bike Data Characteristic is NOT mandatory!");
//    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Indoor Bike Data Characteristic is mandatory!");
/*
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
*/
  ...
  if ( client_FTM_ControlPoint_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Control Point Characteristic is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  if ( client_FTM_Status_Chr.discover() )
  {
    ...
    DEBUG_PRINTLN("Not Found! Disconnecting since Client FTM Status Characteristic is mandatory!");
    // MANDATORY so disconnect   
    Bluefruit.disconnect(conn_handle);
    return;
  }
  ...
  // Only when the Central (Controlling App/Zwift) is connected enable notify/indicate on all Chars
  if(Laptop.IsConnected) { Client_Enable_Notify_Indicate(); }
...
void server_connect_callback(uint16_t conn_handle)
{
  ...
  } else { DEBUG_PRINTLN(">>>> Notice: Trainer is not connected! <<<<"); }
...
void server_cccd_callback(uint16_t conn_handle, BLECharacteristic* chr, uint16_t cccd_value)
{
    ...
    DEBUG_PRINTF("Central Device Updated CCCD to: [%d] --> ", cccd_value);
    ...
    if (chr->uuid == server_FTM_Status_Chr.uuid) { 
        if (chr->notifyEnabled(conn_handle)) { 
          DEBUG_PRINT("Server FTM: Status 'Notify' enabled");
        } else {
          DEBUG_PRINT("Server FTM: Status 'Notify' disabled");
        }
    }
    ...
    if (chr->uuid == server_NUS_TXD_Chr.uuid) { 
        if (chr->notifyEnabled(conn_handle)) { 
          DEBUG_PRINT("Server NUS: TXD 'Notify' enabled"); DEBUG_PRINTLN();
          // Send persistent stored values NOW to Mobile Phone for correct Settings!
          Server_Sends_NUS_TXD_Persistent_Settings();
        } else {
