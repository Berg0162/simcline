...
/* 
 *  This Feather-nRF52840 code scans for the CPS, CSC and FTMS
...
bool clientIsReadyToOperate = false;
...
// exposed SVC/Char's and empty (zero) readings of Char values! Final result: connection fails!
// - Zwift Hub will NOT pass 100%-error-free with a value of poll_delay lower than 250 ms !!
unsigned long poll_delay = 20;   // Unit is milliseconds
// The assumption is that critical connection parameters are not well aligned... Trainer needs 
// slightly different values for a connection than the Client is actually imposing!
// --> Activate when you experience regular failing during the SVC/Char discovery phase
// Uncomment "#define REQUEST_NEW_CONNECTION_PARAMETERS" to activate
//#define REQUEST_NEW_CONNECTION_PARAMETERS
#ifdef REQUEST_NEW_CONNECTION_PARAMETERS
// If behavior persists then increase stepwise the value of REQUESTEDINTERVAL between 24 (default) and 38 
#define REQUESTEDINTERVAL 24 // in units of 1.25 ms -> (24*1.25 = 30 ms)
#endif
//---------------------------------------------------------------------------------------------------
...
const uint8_t client_CP_Feature_Str_Len = 20;
const char* client_CP_Feature_Str[client_CP_Feature_Str_Len] = { 
...
const uint8_t client_CSC_Feature_Str_Len = 3;
const char* client_CSC_Feature_Str[client_CSC_Feature_Str_Len] = {"Wheel rev supported", "Crank rev supported", "Multiple locations supported"};
...
// DATA set -----------------------------------------------------
// set (Zwift Volcano Circuit) with Control and 2D Resistance data to be sent to Control Point
...
// Allow for extra memory usage the nRF52840 has plenty!
  ...
  /* Overview of default connection settings
  ...
  // Default number of Handle Value Notifications that can be queued for transmission.
  // BLE_GATTS_HVN_TX_QUEUE_SIZE_DEFAULT (1)
  // Default number of Write without Response that can be queued for transmission.
...
// -----------------------------------------------------------
  ...
  // Wait for client_connect_callback to finish completely!
  while(!clientIsReadyToOperate) {
    yield();
  }
...
void loop()
{
 if ( clientIsReadyToOperate ) 
 {
  // If time is there, send values of Indoor Bike Simulation Parameters to 
  ...
  if(millis() > TimeInterval) 
  {
    ...
    // Transfer multidimensional data to CPData buffer
...
void Client_Start_Scanning(void)
{
  ...
  // Set Min and Max Connection Interval in units of 1.25ms -> (16*1.25 = 20ms) -> (24*1.25 = 30ms)
  Bluefruit.Central.setConnInterval(16, 24); // default: min/max 16/24   test/vary between min:16 - max:38
  Bluefruit.Scanner.setRxCallback(client_scan_callback);
  Bluefruit.Scanner.restartOnDisconnect(false); // default -> true !!! in this stage we do not want to RESTART -> false
  Bluefruit.Scanner.filterRssi(-80);   // Only invoke callback when RSSI >= -80 dBm so get close to your device !!
  Bluefruit.Scanner.setInterval(160, 80); // Scan interval and window in units of 0.625 ms
...
void client_connect_callback(uint16_t conn_handle)
{
  if(conn_handle == BLE_CONN_HANDLE_INVALID) { 
    DEBUG_PRINTLN(">>> ERROR Invalid Connection Handle!");
  ...
  uint16_t ConnectionInterval = connection->getConnectionInterval();
  DEBUG_PRINTF("ConnectionInterval: [%d] ", ConnectionInterval);
  uint16_t SlaveLatency = connection->getSlaveLatency();
  DEBUG_PRINTF("SlaveLatency: [%d] ", SlaveLatency);
  uint16_t SupervisionTimeout = connection->getSupervisionTimeout();
  ...
  uint8_t requestConnInterval = REQUESTEDINTERVAL; 
  // Connection Interval in units of 1.25ms = ## (vary!), Latency = 0 (keep!), Supervision Timeout in units of 10ms = 400 (keep!)
  if( connection->requestConnectionParameter(requestConnInterval, 0, 400) ) {
    requestConnInterval *= 1.25; // convert to ms
    DEBUG_PRINTF(">>> Request NEW Connection Parameters! Interval: %d ms Latency: 0 Supervision Timeout: 4000 ms\n", requestConnInterval);
    poll_delay = requestConnInterval; // Synchronize SVC/CHR Discovery calls with Connection Interval
  }
  // ------------------------------------------------------------------------------------------------------------------------------
#endif
...
//   b22:32   = Reserved
  ...
  DEBUG_PRINTLN("] ");
  for (int i = 0; i < client_CP_Feature_Str_Len; i++) {
...
bool client_CSC_connect_callback(uint16_t conn_handle) 
{
  ...
  // check for client CSC Characteristics when the client CSC Service is existing
  ...
  if ( client_CSC_Feature_Chr.discover() ) //   UUID16_CHR_CSC_FEATURE
  {
    ...
    DEBUG_PRINTLN("] ");
    for (int i = 0; i < client_CSC_Feature_Str_Len; i++) {
