...
// Test Data with Control and 2D Resistance data
uint8_t ControlPointMessageCount = 0;
/* TEST DATA A --> Zwift Volcano Circuit ----------------------
// END TEST DATA set -----------------------------------------*/
// TEST Data B --> Rolling Hills
...
{0x11,0x00,0x00,0x00,0x00,0x28,0x33,0x00},// Value 00 00 ->  0.00
{0x11,0x00,0x00,0x64,0x00,0x28,0x33,0x00},// Value 64 00 ->  1.00
{0x11,0x00,0x00,0xC8,0x00,0x28,0x33,0x00},// Value C8 00 ->  2.00
{0x11,0x00,0x00,0x2C,0x01,0x28,0x33,0x00},// Value 2C 01 ->  3.00
{0x11,0x00,0x00,0x90,0x01,0x28,0x33,0x00},// Value 90 01 ->  4.00
{0x11,0x00,0x00,0x90,0x01,0x28,0x33,0x00},// Value 90 01 ->  4.00
{0x11,0x00,0x00,0x2C,0x01,0x28,0x33,0x00},// Value 2C 01 ->  3.00
{0x11,0x00,0x00,0xC8,0x00,0x28,0x33,0x00},// Value C8 00 ->  2.00
{0x11,0x00,0x00,0x64,0x00,0x28,0x33,0x00},// Value 64 00 ->  1.00
{0x11,0x00,0x00,0x00,0x00,0x28,0x33,0x00},// Value 00 00 ->  0.00
{0x11,0x00,0x00,0x9C,0xFF,0x28,0x33,0x00},// Value 9C FF -> -1.00
{0x11,0x00,0x00,0x38,0xFF,0x28,0x33,0x00},// Value 38 FF -> -2.00
{0x11,0x00,0x00,0x9C,0xFF,0x28,0x33,0x00},// Value 9C FF -> -1.00
{0x11,0x00,0x00,0x00,0x00,0x28,0x33,0x00},// Value 00 00 ->  0.00
{0x11,0x00,0x00,0x64,0x00,0x28,0x33,0x00},// Value 64 00 ->  1.00
{0x11,0x00,0x00,0xC8,0x00,0x28,0x33,0x00},// Value C8 00 ->  2.00
{0x11,0x00,0x00,0x2C,0x01,0x28,0x33,0x00},// Value 2C 01 ->  3.00
{0x11,0x00,0x00,0x90,0x01,0x28,0x33,0x00},// Value 90 01 ->  4.00
{0x11,0x00,0x00,0xF4,0x01,0x28,0x33,0x00},// Value F4 01 ->  5.00
{0x11,0x00,0x00,0x58,0x02,0x28,0x33,0x00},// Value 58 02 ->  6.00
{0x11,0x00,0x00,0x58,0x02,0x28,0x33,0x00},// Value 58 02 ->  6.00
{0x11,0x00,0x00,0xF4,0x01,0x28,0x33,0x00},// Value F4 01 ->  5.00
{0x11,0x00,0x00,0x90,0x01,0x28,0x33,0x00},// Value 90 01 ->  4.00
{0x11,0x00,0x00,0x2C,0x01,0x28,0x33,0x00},// Value 2C 01 ->  3.00
...
bool client_HeartRate_Connect(void)
{
    ...
    if(pRemote_HR_Measurement_Chr->canNotify()) {
       pRemote_HR_Measurement_Chr->subscribe(notifications, client_HR_Measurement_Notify_Callback);
    } else {
...
bool client_FitnessMachine_Connect(void)
{
    ...
    if(pRemote_FTM_IndoorBikeData_Chr->canNotify()) {
      pRemote_FTM_IndoorBikeData_Chr->subscribe(notifications, client_FTM_IndoorBikeData_Notify_Callback);
    } else {
    ...
    if (pRemote_FTM_TrainingStatus_Chr == nullptr) {
      ...
      if(pRemote_FTM_TrainingStatus_Chr->canNotify()) {
        pRemote_FTM_TrainingStatus_Chr->subscribe(notifications, client_FTM_TrainingStatus_Notify_Callback);
      } else {
    ...
    if(pRemote_FTM_ControlPoint_Chr->canIndicate()) {
      pRemote_FTM_ControlPoint_Chr->subscribe(indications, client_FTM_ControlPoint_Indicate_Callback);
    } else {
    ...
    if(pRemote_FTM_Status_Chr->canNotify()) {
      pRemote_FTM_Status_Chr->subscribe(notifications, client_FTM_Status_Notify_Callback);
    } else {
...
void controlPointDataDecode(uint8_t CPData[8]){
    uint8_t OPCODE = CPData[0];
    uint8_t OCTETS[6];
    memcpy(&OCTETS, &CPData[1], 6);
    switch(OPCODE) {
      ...
      short ws = (OCTETS[0] << 8) + OCTETS[1]; 
      float wind_speed = ws / 1000.0;
      // Short is 16 bit signed, so a negative grade is correctly converted from two bytes to signed value. Highest bit is sign bit
      short gr = (OCTETS[3] << 8) + OCTETS[2]; 
      float grade = gr / 100.0;
      float crr = OCTETS[4] / 10000.0;
      float cw = OCTETS[5] / 100.0;
