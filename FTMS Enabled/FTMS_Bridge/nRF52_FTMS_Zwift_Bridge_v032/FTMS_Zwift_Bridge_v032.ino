...
void setup()
{
  ...
  DEBUG_PRINTLN("Feather nRF52840 MITM supporting: CPS, CSC, HRM and FTMS");
  DEBUG_PRINTLN("-------------------- Version 03.2 ----------------------");
  DEBUG_PRINTF("Discover polling delay setting: [%d] ms\n", poll_delay);
#endif
// ------------------------------------------------------
...
// -----------------------------------------------------------
  ...
  // Wait until client_connect_callback has completely finished
  while(!Trainer.IsConnected) { 
    yield();
  }
...
void Client_Start_Scanning(void)
{
  ...
  */
  ...
  Bluefruit.Scanner.restartOnDisconnect(true); // default is true !!! this stage we do not want to RESTART
...
#ifdef DEBUG
  DEBUG_PRINTLN("Found advertising Peripheral with FTMS enabled! See Raw data packet:");
...
void client_disconnect_callback(uint16_t conn_handle, uint8_t reason)
{
  ...
  DEBUG_PRINTLN("Client-side starts scanning again!");
}
...
void server_startADV(void)
{
  ...
  // Set Peripheral Preferred Connection Interval Min & Max, units in 1.25ms
  Bluefruit.Periph.setConnInterval(16, 200); // (16-24 20-30 ms)
...
void server_FTM_ControlPoint_Chr_callback(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
  ...
  switch (server_FTM_Control_Point_Data.values.OPCODE) {
    ...
    case ftmcpSetIndoorBikeSimulationParameters: {
        ...
        DEBUG_PRINT("Set Indoor Bike Simulation Parameters --> ");
