...
// Version 1.1 NOW keep little endian format (24 bit) in IBData in the transfer to uint32 Total Distance
// Version 1.2 NOW code ignores (flags & 2) true or false --> ALWAYS(?) is Average Speed present in IBData (Elite Direto)
// Version 1.3 (1.1 turned out NOT to be solved) --> corrected algorithm for T.Distance (in 24 bit in IBData) to uint32 T.Distance
...
void setup()
{
  ...
  Serial.println(" Feather nRF52 Client/Central: FTMS, CPS and CSC");
  Serial.println("---------------- Version 01.3 -------------------");
  Serial.println("Initialise the Bluefruit nRF52 module: Central");
#endif
  ...
  // Only now enable Client (Trainer) CPS, CSC and FTMS data streams...
  Client_Enable_Notify_Indicate(); 
#ifdef DEBUG   
  Serial.println("Client is Up and Running!");
#endif
...
void loop()
{
  // If time is there send test data to the FTM Control Point to drive the FTM
  if(millis() > TimeInterval) {
    if(ControlPointMessageCount > 31) { ControlPointMessageCount = 0; } // start all over again!
    uint8_t CPData[8] = {};
#ifdef DEBUG
    Serial.print("Control Point Data Written: [ ");
#endif
...
#ifdef DEBUG  
    Serial.println("Couldn't enable indicate for Client CP Control Point Characteristic. Increase DEBUG LEVEL for troubleshooting");
#endif
  }
...
#ifdef DEBUG
    Serial.println("Couldn't enable notify for Client FTM Control Point Characteristic. Increase DEBUG LEVEL for troubleshooting");
#endif
  }
...
//    }
  ...
  if ((flags & 16) != 0) {
    ...
    uint8_t byte_MSO = (uint8_t)IBDDataBuf[offset+2]; // byte 3 MSO
    // Little endian format, transfer 24 bit to 32 bit variable
    uint32_t tot_Distance = ( (byte_LSO << 24) | (byte_MID << 16) | (byte_MSO << 8) | 0x00 ); // 4 bytes in Little Endian format
...
void client_scan_callback(ble_gap_evt_adv_report_t* report)
{
  ...
  // Connect to device with Client CPS service in advertising
#ifdef DEBUG  
  Serial.println("Advertising Client with FTMS, CPS or CSC is found! ... Raw data packet:");
...
#ifdef DEBUG
  Serial.println("Now checking mandatory Client Services and Characteristics!");
...
#ifdef DEBUG
        Serial.printf("Found Client Manufacturer:  [%s]\n", client_DIS_Manufacturer_Str);
#endif
      }
    }
    //  2
...
#ifdef DEBUG
        Serial.printf("Found Client Model Number:  [%s]\n", client_DIS_ModelNumber_Str);
#endif
      }
    }
    //  3
...
#ifdef DEBUG
        Serial.printf("Found Client Serial Number: [%s]\n", client_DIS_SerialNumber_Str);
#endif
      }
    }
  }
...
#endif
  // If FTM Training Status is not found, disconnect, resume scanning, and return
...
#ifdef DEBUG 
    Serial.println("Not Found! Client FTM Supported Resistance Level Range Characteristic is NOT mandatory!"); 
#endif
    /*
