...
#ifdef DEBUG
     Serial.printf("Peripheral device (Trainer) found that has FTMS [%04X] enabled!\n", UUID16_SVC_FITNESS_MACHINE, HEX);
#endif
  } else { return; } // Keep scanning for FTMS trainer !!
...
#ifdef DEBUG  
  Serial.println("Advertising Peripheral with FTMS, CPS and CSC is found! See Raw data packet:");
...
void client_connect_callback(uint16_t conn_handle)
{
  ...
  if ( client_CyclingPower_Service.discover(conn_handle) )
  {
    ...
    Serial.printf("Found it! CPS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
#ifdef DEBUG
    Serial.println("Not Found! Disconnecting since Client Cyling Power Service is mandatory!");
#endif
  ...
  if ( client_CyclingSpeedCadence_Service.discover(conn_handle) ) //   UUID16_SVC_CYCLING_SPEED_AND_CADENCE
  {
    ...
    uint16_t max_payload = conn->getMtu()-3;
    uint16_t data_length = conn->getDataLength();
    Serial.printf("Found it! CSCS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
#ifdef DEBUG
...
#ifdef DEBUG
         Serial.printf("Found Appearance: [%d]\n", client_GA_Appearance_Value);
#endif
      }     
  }
  ...
  if ( client_FitnessMachine_Service.discover(conn_handle) )
  {
    ...
    uint16_t max_payload = conn->getMtu()-3;
    uint16_t data_length = conn->getDataLength();
    Serial.printf("Found it! FTMS Max Payload: %d Data Length: %d\n", max_payload, data_length);
#endif
  } else {
#ifdef DEBUG
...
void server_FTM_ControlPoint_Chr_callback(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
  ...
  // NO TREATMENT OF COMMAND !!!
  //  client_FTM_ControlPoint_Chr.write_resp(data, len); Seems to cause hickups in response stream // Just pass on and process later! 
...
void server_CP_ControlPoint_Chr_callback(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
  ...
  // client_CP_ControlPoint_Chr.write_resp(data, len); Not sure this is the right choice! // Just pass on and process later!
...
#endif
  TimeInterval = millis() + TIME_SPAN; // ADD just enough delay
  while(!server_NUS_TXD_Chr.notifyEnabled()) { // Wait until NUS TXD is 'Notify' enabled by Smartphone
    if( millis() > TimeInterval ) {
#ifdef DEBUG
      Serial.println("Central (Phone) Error: Timeout on NUS Txd 'Notify'"); 
#endif
      Bluefruit.disconnect(conn_handle); // No choice --> disconnect and start advertising again!
      return;
    }
  }
}
