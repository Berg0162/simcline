...
/* 
Fitness Machine Features (byte 1-4):  Test      Direto XR
...
// Timing variables
#define TIME_SPAN_TO_SETTLE  2500 // Time span in millis 1000 -> 1 sec
#define IBD_TIME_SPAN        1000 // Time span (delay) for sending Indoor Bike Data
#define CP_CSC_TIME_SPAN      300 // Time span (delay) for sending CP and CSC data
...
void setup()
{
  ...
  Serial.println(" Feather nRF52 Server/Peripheral: CPS, CSC and FTMS");
  Serial.println("-------------------  Version 02 ---------------------");
  // Initialise the Bluefruit module
  Serial.println("Initialise the Bluefruit nRF52 module: Server (Peripheral)");
#endif
  ...
  server_startADV();
#ifdef DEBUG
  Serial.println("Server is advertising: CPS, CSC and FTMS");
#endif
  while (Bluefruit.Advertising.isRunning()) { // ONLY advertise!
    yield();
  }
  TimeInterval = millis() + TIME_SPAN_TO_SETTLE; // ADD just enough DELAY
  // wait enough time or go on when server is connected and set!
  while ( (TimeInterval > millis()) ) { 
    yield();
    }
#ifdef DEBUG   
  Serial.println("Server (Peripheral) is Up and Running!");
#endif
...
void server_connect_callback(uint16_t conn_handle)
{
  ...
  Serial.println();
  Serial.println("Waiting for Central Device to set 'Notify' enabled for relevant Characteristics...");
#endif
}
...
#ifdef DEBUG
  Serial.printf("Server disconnected from Central Device, Reason: [%d]\n", reason);
