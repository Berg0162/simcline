...
Version 1.2.1
Deleted while(pBLEScan->isScanning()) after client_start_scanning
Version 1.4
...
Inserted code for handling connect and reconnect differently -> client_connect_callback
...
bool isClientCPWriteAcknowledged = true;                 //  CP Write-Response synch test
...
BLEAdvertisedDevice* wahooDevice = nullptr;
...
bool client_Connect_Callback(void) {
  ...
  #ifdef DEBUG_WAHOO_CONTROLPOINT_OPCODE_DATA 
  cpWriteTime = millis(); // Set start time to measure later the response time
  #else
  DEBUG_PRINTLN("");
  #endif
  ...
  while(1) { 
      if(xTaskClientWriteWithResponseDataReady) {
          if(!pRemote_Wahoo_ControlPoint_Chr->writeValue(xTaskClientWriteWithResponseData, true))
              DEBUG_PRINTLN(">>> Error: Failed to write characteristic -> Return Code = 0x06");
...
void onWrite(BLECharacteristic *pCharacteristic) {
  ...
  // NO TREATMENT OF COMMAND !!!
  if(Trainer.IsConnected && isClientCPWriteAcknowledged) {
    if(xTaskClientWriteWithResponseDataReady == false) {   // Previous xTask Write With Response is finished 
        ...
        isClientCPWriteAcknowledged = false;               // Set Response to false
        vTaskResume(xTaskClientWriteWithResponseHandle);   // Resume xTask and write the data a.s.a.p.
        eventCnt = 0;  
    }
  } else { // Training App keeps sending CP-Data when Client/Trainer is disconnected or unresponsive!!
    if(eventCnt++ < 2) { // Limit active response to this state
      DEBUG_PRINTLN("\n>>> Wahoo Trainer is unresponsive!");
    }
