...
void setup()
{
  ...
  Serial.println("Wahoo Kickr Client with Cycling Power Service only");
...
void Set_SimMode(void)
{
  ...
  if (client_CPWT_found) { 
    ...
    Serial.println("Command:  [ 43 20 1C 04 00 70 01 ] --> setSimMode Weight: 72.0 RRC: 0.004000 WRC: 0.368000");
    }
}
...
void Set_SimGrade(float gr)
{
  ...
  if (client_CPWT_found) { 
    ...
    Serial.printf("] --> setSimGrade Grade: %5.2f%%", gr);
    Serial.println();
    }
}
...
void client_connect_callback(uint16_t conn_handle)
{
  ...
  if ( !client_cpwt.discover() ) //   CYCLING_POWER_WAHOO_TRAINER
  {
    ...
    Serial.println("Not Found! NO TRAINER CONTROL!"); 
  ...
  Serial.print("Discovering Client Power Feature characteristic ... ");
...
void client_disconnect_callback(uint16_t conn_handle, uint8_t reason)
{
  ...
  Serial.print("Client Disconnected Peripheral Device, reason = 0x"); Serial.println(reason, HEX);
}
