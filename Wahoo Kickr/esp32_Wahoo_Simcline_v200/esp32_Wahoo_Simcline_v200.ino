 ...
 *  7) Pair: Power Source, Cadence and Resistance one after another with <SIM32>
...
Version 1.2.1
Deleted while(pNimBLEScan->isScanning()) after client_start_scanning
Version 1.4
Changed code sequence in onResult before BLEDevice::getScan()->stop(); is called
Extende with CP and Response timing data code for handling DEBUG_WAHOO_CONTROLPOINT_OPCODE_DATA
Inserted code for handling connect and reconnect differently -> ClientConnectServer
Inserted a new xTaskClientWriteWithResponse for Wahoo CP write-with-response with suspend-resume mechanism
Version 2.0
...
/* NimBLERemoteCharacteristic* pRemote_CP_ControlPoint_Chr;   // Indicate, Write */
/* ---------------------------------------------------------------------------------------------------------------
...
long cpWriteTime;           // The message response time after calling xTaskClientWriteWithResponse!
#endif
...
const NimBLEAdvertisedDevice* wahooDevice = nullptr;
...
#define MEASUREOFFSET 0 // 50 = about 1.25 cm
// Raw Grade Value Minimally (Mechanically: the lowest position of wheel axis)  19000 is equiv. of 10% downhill road grade
...
#define MINPOSITION 10 // 270 // 265 // VL6180X highest value top microswitch activated to mechanically stop operation
#define MAXPOSITION 195 //470 // 535 // VL6180X lowest value bottom microswitch activated to mechanically stop operation
 ...
 */
  ...
  */
  ...
  ShowOnOledLarge("SIMCLINE", "Wahoo", "v01.4", 500);
  ...
  if(!Trainer.IsConnected) {
    DEBUG_PRINTLN(">>> Failed to connect Wahoo Trainer! Reset ESP32 and try again!");
    while(1) {delay(100);}
  }
} // End of setup.
...
void SetNewRawGradeValue(float RoadGrade)
{
        ...
        // --------------------------------------------------------------------------------------------------
#ifndef DEBUG_WAHOO_CONTROLPOINT_OPCODE_DATA
...
void client_Wahoo_ControlPoint_Indicate_Callback(NimBLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify)
{
  ...
  // Notice: Zwift does not check at all this acknowledgement, it is (surprisingly) NOT critical for a successful protocol handling!!!
  if((Laptop.IsConnected)) {   
    server_Wahoo_ControlPoint_Chr->setValue(pData, length);
    server_Wahoo_ControlPoint_Chr->indicate(); // Just pass on and process later!
  } 
#ifdef DEBUG_WAHOO_CONTROLPOINT_OPCODE_DATA 
  DEBUG_PRINTF(" [ACK:%4d]\n", (millis()-cpWriteTime) );  // testing only!
#endif
...
void client_CSC_Measurement_Notify_Callback(NimBLERemoteCharacteristic* pBLERemoteCharacteristic, uint8_t* pData, size_t length, bool isNotify)
{
...
bool ClientConnectServer(void) {
  // Handle first time connect AND a reconnect. One fixed Wahoo Trainer to account for!
  if(pClient_Wahoo == nullptr) { // First time -> create new pClient and service database!
    pClient_Wahoo = NimBLEDevice::createClient(); 
    pClient_Wahoo->setClientCallbacks(new client_Connection_Callbacks());
    // First Time Connect to the Wahoo Trainer (Server/Peripheral)
    hasConnectPassed = pClient_Wahoo->connect(wahooDevice, true);   // Delete attribute objects and Create service database
  } else if(pClient_Wahoo == NimBLEDevice::getDisconnectedClient()) { // Allow for a streamlined reconnect
      // Reconnect to the disconnected Wahoo Trainer (Server/Peripheral)
      hasConnectPassed = pClient_Wahoo->connect(wahooDevice, false);  // Just refresh the service database 
    } 
...
void client_Connection_Callbacks::onConnect(NimBLEClient* pClient) {
    Trainer.PeerName = wahooDevice->getName().c_str();
    ...
    DEBUG_PRINTF("ESP32 Client connects to Server device with Name: [%s] MAC Address: [%s] Handle: [%d] MTU: [%d]\n", \
...
void CharacteristicCallbacks::onSubscribe(NimBLECharacteristic* pCharacteristic, NimBLEConnInfo& connInfo, uint16_t subValue) {
        std::string str = "Central";
        if(subValue == 0) {
            str += " Unsubscribed to Char:";
        }else {
            str += " Subscribed to Char:";
        } 
        DEBUG_PRINTF("[%d] %s", subValue, str.c_str());
        str = std::string(pCharacteristic->getUUID()).c_str();
        DEBUG_PRINTF(" [%s] -> ", str.c_str());
        if(pCharacteristic->getUUID() == UUID16_CHR_CYCLING_POWER_MEASUREMENT) DEBUG_PRINTLN("CPS Measurement");
        if(pCharacteristic->getUUID() == UUID16_CHR_CSC_MEASUREMENT) DEBUG_PRINTLN("CSC Measurement");
};
...
void onWrite(NimBLECharacteristic *pCharacteristic, NimBLEConnInfo& connInfo) {
  ...
  DEBUG_PRINTF("[CPI:%5d] ", cpInterval); // The elapsed time between two Control Point events
  DEBUG_PRINTF("-> Rec'd Raw Wahoo Control Point Data [len: %d] ", WahooCPDataLen);
#endif
  ...
  switch(server_Wahoo_Control_Point_Data.values.OPCODE) {
    ...
    case setSimMode : {
      ...
      DEBUG_PRINTF(" --> Weight: %0.2f RRC: %f WRC: %f ", weight, rrc, wrc);
      if(eventCnt) DEBUG_PRINTLN("");
      break;
    }
#endif
    case setSimGrade: {
  ...
  void onSubscribe(NimBLECharacteristic* pCharacteristic, NimBLEConnInfo& connInfo, uint16_t subValue) {
        ...
        str = std::string(pCharacteristic->getUUID()).c_str();
        DEBUG_PRINTF(" [%s] -> ", str.c_str());
        DEBUG_PRINTLN("Wahoo CPS Control Point");
  }; // onSubscribe
};
...
*/  
  void onSubscribe(NimBLECharacteristic* pCharacteristic, NimBLEConnInfo& connInfo, uint16_t subValue) {
        ...
        str = std::string(pCharacteristic->getUUID()).c_str();
        DEBUG_PRINTF(" [%s] -> ", str.c_str());
        DEBUG_PRINTLN("NUS Transmit Data");
...
if(IsEnable) { // Enable Client Char's
  if( pRemote_Wahoo_ControlPoint_Chr != nullptr) { // Check: Is it exposed?
            ...
            DEBUG_PRINT("Client sends to Wahoo Control Point: [ 20 EE FC ] -> Unlock Command Key");
#ifdef DEBUG_WAHOO_CONTROLPOINT_OPCODE_DATA 
            cpWriteTime = millis(); // Set current to measure acknowledged time later
#else DEBUG_PRINTLN("");
#endif
...
void loop() { // loop() is used to start sort of Callback functions
 ...
 // If the flag "DoCallClientDisable" is true, we disable Notify and Indicate on ALL Client Char's of BLE server!
  ...
  if (doClientConnectCall) {
    doClientConnectCall = false;
    if(!ClientConnectServer()) {
      DEBUG_PRINTLN(">>> Failed to reconnect Wahoo Trainer!");
      NimBLEDevice::deleteClient(pClient_Wahoo); // Delete client object and clear from list
      pClient_Wahoo = nullptr;   // Clear to null
