/***********************************************************************************
 ...
 22/01/2022 Version v031 works with both types of nRF52 Feather processors
 12/02/2022 3.2 RGVMIN and MAX setting at just one location, calculation refined, Do-While loop UpDownmovement,
            flat road setting
 ***************************  VERSION 3.2 *****************************************/
...
// Correction for measuring/reflection plane difference and midth wheel axis position (2 cm offset is an MEASUREOFFSET of 200)
...
#define MINPOSITION 265 // 265 // VL6180X highest value top microswitch activated to mechanically stop operation
#define MAXPOSITION 535 // 535 // VL6180X lowest value bottom microswitch activated to mechanically stop operation
// -------------------------- WARNING ------------------------------------------------------------
...
void setup()
{
  ...
  ShowOnOledLarge("", "SIMCLINE", "3.2", 500);
...
void ControlUpDownMovement(void) {
  ...
  Serial.printf("RawgradeValue: %05d ", RawgradeValue, DEC); Serial.printf(" TargetPosition: %03d\n", TargetPosition, DEC);
#endif
  uint8_t OnOffsetAction = 0;
do { // Run the loop until target position is reached, BLE channels can interrupt and change target position meanwhile !!
  digitalToggle(LED_RED); // blinking red led means we are working....
  ...
  TargetPosition = map(RawgradeValue, RGVMIN, RGVMAX, MAXPOSITION, MINPOSITION);
  lift.SetTargetPosition(TargetPosition);
  OnOffsetAction = lift.GetOffsetPosition(); // calculate offset and determine which action is needed
...
} while ( (OnOffsetAction == 1) || (OnOffsetAction == 2) ); // (OnOffsetAction != 0); // Run the loop until target position is reached!
